...
import { Transaction, TRANSFER_FEE_PERCENTAGE } from './types';
import { GIFT_ITEMS } from './giftItems';
...
export class DreamCoinBank {
  ...
  public transferCoins(senderId: string, recipientId: string, amount: number): boolean {
    ...
    const serviceFee = amount * (TRANSFER_FEE_PERCENTAGE / 100);
  ...
  public deductTransactionFee(userId: string, amount: number, reason: string): boolean {
    ...
    if (userBalance < amount && amount > 0) { // Only check if we're deducting (positive amount)
