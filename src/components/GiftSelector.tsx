...
const GiftSelector: React.FC<GiftSelectorProps> = ({
  ...
  const filteredGifts = selectedCategory === 'all' ? availableGifts : availableGifts.filter(gift => gift.category === selectedCategory);
      ...
      <DialogContent className="bg-zinc-900 border-zinc-800 text-white sm:max-w-md">
        ...
        <Tabs defaultValue={selectedCategory} onValueChange={setSelectedCategory}>
          <TabsList className="bg-zinc-800 grid grid-flow-col auto-cols-fr overflow-x-auto scrollbar-hide">
            {categories.map(category => <TabsTrigger key={category} value={category} className="capitalize data-[state=active]:bg-rose-500 data-[state=active]:text-white">
                {category}
              </TabsTrigger>)}
          </TabsList>
          ...
          <TabsContent value={selectedCategory} className="mt-4">
            <div className="grid grid-cols-3 gap-3">
              ...
              return <div key={gift.id} className={`rounded-lg p-2 border-2 cursor-pointer flex flex-col items-center justify-center transition-colors
                      ${selectedGift?.id === gift.id ? 'border-rose-500 bg-rose-500/10' : 'border-zinc-800'}
                      ${!canAfford ? 'opacity-50 cursor-not-allowed' : 'hover:border-rose-500/50'}`} onClick={() => canAfford && handleGiftSelect(gift)}>
                    <div className="w-14 h-14 flex items-center justify-center">
                      <img src={gift.imageUrl} alt={gift.name} className="max-w-full max-h-full" />
                    </div>
        ...
        <div className="flex items-center justify-end gap-2 mt-4">
          ...
          <Button variant="default" onClick={handleConfirmSelection} disabled={!selectedGift || balance < (selectedGift?.price || 0)} className="bg-rose-500 hover:bg-rose-600">
