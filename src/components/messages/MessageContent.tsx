  ...
  switch (message.type) {
    ...
    case MessageType.MEDIA:
      if (message.mediaType === 'image') {
        return <div className="mt-2">
            <img src={message.mediaUrl} alt="Image" className="max-w-[200px] rounded-lg" />
          </div>;
      } else if (message.mediaType === 'video') {
        return <div className="mt-2">
            <video src={message.mediaUrl} controls className="max-w-[200px] rounded-lg" />
          </div>;
    ...
    case MessageType.GIFT:
      return <div className="mt-2 flex flex-col items-center">
          <p>{message.content}</p>
          <img src="/user-uploads/gifts/heart.png" alt="Gift" className="w-24 h-24 mt-2" />
        </div>;
