    ...
    <div className="flex items-start gap-2 mb-4">
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-1">
          ...
          {timestamp && <span className="text-xs text-white/70">{timestamp}</span>}
          {!isUser && <span className="text-xs px-1.5 py-0.5 rounded bg-slate-900/30 text-white/70">v1</span>}
        </div>
        <p className="text-sm text-white/90">{content}</p>
        {!isUser && (
          <div className="flex items-center mt-1 gap-2">
            <button className="flex items-center gap-1 px-2 py-0.5 text-xs text-white bg-adam-gray/50 rounded hover:bg-adam-gray/70 transition-colors">
