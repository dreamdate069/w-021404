...
import { Heart, Mail, Lock, User, Calendar, MapPin, Eye, EyeOff } from 'lucide-react';
...
const AuthPage = () => {
  ...
  useEffect(() => {
    if (user) {
      const redirectTo = locationState.state?.from?.pathname || '/explore';
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      const userData = {
        ...
        gender: gender,
        nickname: `${firstName}${Math.floor(Math.random() * 1000)}`
      };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-purple-50 dark:from-zinc-900 dark:via-zinc-800 dark:to-zinc-900 flex items-center justify-center p-4">
      <Card className="w-full max-w-md bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm border-rose-200 dark:border-zinc-700">
